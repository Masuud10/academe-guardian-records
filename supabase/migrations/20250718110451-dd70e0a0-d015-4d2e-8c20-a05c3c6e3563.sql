-- PHASE 1: Complete Rebuild of HR Role Database Foundation (Fixed)
-- This is a forensic, deep-dive rebuild to fix HR login failures

-- 1. First, clean up admin users from the school application
-- Move admin users to inactive status so they can't login to school app
UPDATE public.profiles 
SET status = 'inactive'
WHERE role IN ('edufam_admin', 'elimisha_admin');

-- 2. Clean up and rebuild CHECK constraints  
...
-- Create the definitive role constraint for SCHOOL APPLICATION ONLY
...
CHECK (
  role = ANY (ARRAY[
    ...
    'hr'::text,
    'edufam_admin'::text,  -- Keep for admin app compatibility but set inactive
    'elimisha_admin'::text  -- Keep for admin app compatibility but set inactive
  ])
);

-- 3. Fix the critical RLS policies that are blocking HR login
...
-- 4. Fix school assignment constraints for HR
...
CHECK (
  -- School roles require school_id (admins can be null for backwards compatibility)
  (role IN ('school_director', 'principal', 'teacher', 'parent', 'finance_officer', 'hr') AND school_id IS NOT NULL)
  OR 
  (role IN ('edufam_admin', 'elimisha_admin'))
);

-- 5. Ensure HR user has correct password hash  
UPDATE auth.users 
SET encrypted_password = crypt('HRPassword123!', gen_salt('bf'))
WHERE email = 'hr@edufam.com';

-- 6. Verify HR user profile is correctly set up
UPDATE public.profiles
SET 
  role = 'hr',
  status = 'active',
...
-- 7. Test the setup with a validation query
DO $$
DECLARE
    hr_check record;
BEGIN
