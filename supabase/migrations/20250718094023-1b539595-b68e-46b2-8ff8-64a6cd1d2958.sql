-- Update all existing school_owner roles to school_director in user_login_details table first
DO $$
BEGIN
...
-- Now update the check constraint on user_login_details table to include school_director
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_login_details') THEN
