...
-- FIXED VERSION: Using MD5 instead of gen_salt which may not be available
...
DECLARE
    ...
    password_hash TEXT;
BEGIN
    -- Create password hash using MD5 (simpler approach)
    password_hash := md5('HRPassword123!' || user_email);
...
BEGIN
    ...
    IF (verification_result->>'total_hr_users')::int >= 1 THEN
        RAISE NOTICE '✅ HR users are properly configured!';
...
DECLARE
    ...
    password_hash TEXT;
BEGIN
    -- Create password hash using MD5
    password_hash := md5('HRPassword123!' || user_email);
